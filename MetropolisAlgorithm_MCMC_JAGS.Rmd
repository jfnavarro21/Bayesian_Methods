...
### 5.3.1 Separate samples

Create 2 data lists for JAGS, one for control group and one for treated group
...
```{r}
# assign the classes (Treatment or control) :s
(s <- as.numeric(myData$s))
# Select only the treatment observations : gr.Treat
(gr.Treat <- myData[s==2,])
# extract the y values of treatment cases : y.Treat
(y.Treat <- gr.Treat$y)
# calculate the length of y.Treat: Ngr.Treat
(Ngr.Treat <- length(y.Treat))
# Assign the control group: gr.Control
(gr.Contr <- myData[s==1,])
# extract the y values of the control: y.Contr
(y.Contr <- gr.Contr$y)
# length of y.Contr
(Ngr.Contr <- length(y.Contr))
# Calculate the MLE means:MLE.meanTreat, MLE.meanContr
(MLE.meanTreat <- sum(y.Treat)/Ngr.Treat)
(MLE.meanContr <- sum(y.Contr)/Ngr.Contr)
# Create data lists
(dataList.Treat <- list(y=y.Treat,Ntotal=Ngr.Treat))
(dataList.Contr <- list(y=y.Contr, Ntotal=Ngr.Contr))

```
## 5.3.2 Create model description

Create one model description that works for both groups

```{r}
modelString.SingleModel=" 
model {
  for (i in 1:Ntotal) {
    y[i]~dbern(theta)
  }
...
"
writeLines( modelString.SingleModel , con="TEMPmodelSingle.txt" )
```

### 5.3.3 Initialize function

Create function that randomly initializes chains near MLE
...
### 5.3.4 run MCMC for the control group

Create JAGS model, run burn in and simulate Markov chains for the control group

```{r}
#Create model Contr
jagsModel.Contr<-jags.model(file="TEMPmodelSingle.txt",data=dataList.Contr,
...
```{r}
#Update Contr
update(jagsModel.Contr,n.iter=600)

#Run Contr
codaSamples.Contr<-coda.samples(jagsModel.Contr,variable.names=c("theta"),n.iter=3334)
list.samplers(jagsModel.Contr)
```
Analyze chains, assess the convergence
```{r}
summary(codaSamples.Contr)
```
```{r}
coda::traceplot(codaSamples.Contr)
densplot(codaSamples.Contr)
plot(codaSamples.Contr)
autocorr.plot(codaSamples.Contr, ask=F)
effectiveSize(codaSamples.Contr)
gelman.diag(codaSamples.Contr)
gelman.plot(codaSamples.Contr)
```
Find mean values of the parameter
```{r}
lapply(codaSamples.Contr,mean)
# Create histograms
l<-min(unlist(codaSamples.Contr))-.05
h<-max(unlist(codaSamples.Contr))+.05
histBreaks<-seq(l,h,by=.05)
postHist.Contr<-lapply(codaSamples.Contr,hist,breaks=histBreaks)

plot(postHist.Contr[[1]]$mids,postHist.Contr[[1]]$density,type="l",col="black",lwd=2,ylim=c(0,6),ylab="Distribution Density (Contred)",xlab="Theta")
lines(postHist.Contr[[2]]$mids,postHist.Contr[[2]]$density,type="l",col="red",lwd=2)
lines(postHist.Contr[[3]]$mids,postHist.Contr[[3]]$density,type="l",col="blue",lwd=2)
lines(postHist.Contr[[3]]$mids,dbeta(postHist.Contr[[3]]$mids,1+sum(y.Contr),Ngr.Contr-sum(y.Contr)+1),
      type="l",col="green",lwd=3)
legend("topright",legend=c("Chain1","Chain2","Chain3","Theoretical"),col=c("black","red","blue","green"),lwd=2)
```
### 5.3.5 Run MCMC for the treated group

Create JAGS model, run burn in and simulate Markov chains for the control group

```{r}
#Create model Treat
jagsModel.Treat<-jags.model(file="TEMPmodelSingle.txt",data=dataList.Treat,
                            n.chains=3,n.adapt=500)
```

```{r}
#Update Treat
update(jagsModel.Treat,n.iter=600)

#Run Treat
codaSamples.Treat<-coda.samples(jagsModel.Treat,variable.names=c("theta"),n.iter=3334)
list.samplers(jagsModel.Treat)
```
Analyze the chains assess convergence
```{r}
summary(codaSamples.Treat)
coda::traceplot(codaSamples.Treat)
densplot(codaSamples.Treat)
plot(codaSamples.Treat)
autocorr.plot(codaSamples.Treat,ask=F)
effectiveSize(codaSamples.Treat)
gelman.diag(codaSamples.Treat)
gelman.plot(codaSamples.Treat)
# Find mean values of the parameters
lapply(codaSamples.Treat,mean)
# Create histograms
l<-min(unlist(codaSamples.Treat))-.05
h<-max(unlist(codaSamples.Treat))+.05
histBreaks<-seq(l,h,by=.05)
postHist.Treat<-lapply(codaSamples.Treat,hist,breaks=histBreaks)

plot(postHist.Treat[[1]]$mids,postHist.Treat[[1]]$density,type="l",col="black",lwd=2,ylim=c(0,6),ylab="Distribution Density (Treated)",xlab="Theta")
lines(postHist.Treat[[2]]$mids,postHist.Treat[[2]]$density,type="l",col="red",lwd=2)
lines(postHist.Treat[[3]]$mids,postHist.Treat[[3]]$density,type="l",col="blue",lwd=2)
lines(postHist.Treat[[3]]$mids,dbeta(postHist.Treat[[3]]$mids,1+sum(y.Treat),Ngr.Treat-sum(y.Treat)+1),
      type="l",col="green",lwd=3)
legend("topright",legend=c("Chain1","Chain2","Chain3","Theoretical"),col=c("black","red","blue","green"),lwd=2)
```
### 5.3.6 Find HDI of chains for treated and control groups

Use function HDIofMCMC() form [K] to find 95%HDI of the treated and control groups
```{r}
(HDIofChains.Contr<-lapply(codaSamples.Contr,function(z) Theta.Contr=HDIofMCMC(codaSamples.Contr[[1]][,1])))
```
```{r}
(HDIofChains.Treat<-lapply(codaSamples.Treat,function(z)   Theta.Treat=HDIofMCMC(codaSamples.Treat[[1]][,1])))
```
```{r}
plot(HDIofChains.Treat[[1]],c(.2,.2),type="l",col="orange",xlim=c(0,1),ylim=c(0,.8),
     xlab="HDI",ylab="Chain")
lines(HDIofChains.Treat[[2]],c(.4,.4),col="orange")
lines(HDIofChains.Treat[[3]],c(.6,.6),col="orange")
lines(HDIofChains.Contr[[1]],c(.3,.3),col="blue")
lines(HDIofChains.Contr[[2]],c(.5,.5),col="blue")
lines(HDIofChains.Contr[[3]],c(.7,.7),col="blue")

```
HDI at 95% cannot distinguish the two proportions

## 5.4 Compare HDI bounds of hierarchical model and separate modls

```{r}
# Separate
plot(HDIofChains.Treat[[1]],c(.05,.05),type="l",col="orange",xlim=c(0,1),ylim=c(0,1),
     xlab="HDI",ylab="Chain")
lines(HDIofChains.Treat[[2]],c(.15,.15),col="orange")
lines(HDIofChains.Treat[[3]],c(.25,.25),col="orange")
lines(HDIofChains.Contr[[1]],c(.1,.1),col="blue")
lines(HDIofChains.Contr[[2]],c(.2,.2),col="blue")
lines(HDIofChains.Contr[[3]],c(.3,.3),col="blue")

# Hierarchical    
lines(HDIofChainsHierarchi[[1]][,1],c(.4,.4),col="purple")
lines(HDIofChainsHierarchi[[1]][,2],c(.45,.45),col="magenta")
lines(HDIofChainsHierarchi[[2]][,1],c(.5,.5),col="purple")
lines(HDIofChainsHierarchi[[2]][,2],c(.55,.55),col="magenta")
lines(HDIofChainsHierarchi[[3]][,1],c(.6,.6),col="purple")
lines(HDIofChainsHierarchi[[3]][,2],c(.65,.65),col="magenta")
legend("topleft",legend=c("Separate Left",
                          "Separate Right",
                          "Hierarchical Left",
                          "Hierarchical Right"),
       lty=1, col=c("blue","orange","purple","magenta"))
```
